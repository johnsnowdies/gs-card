...
#ifdef LINKED_EGAVGA_BGI
  void
  DIAG_MSG( char* filename ,int line ,char* comment)
          {  printf("\n%s: line %d - %s",filename,line,comment); }
  extern
  void
  egavgaDriverProc(void);     /*for linked EGAVGA.DRV !!!!!!! */
#endif
...
#ifdef LINKED_EGAVGA_BGI
  if(!registerbgidriver(egavgaDriverProc) == 1)  /*for linked EGAVGA.DRV !!!!!!! */
  {
    DIAG_MSG(__FILE__ ,__LINE__ ,"Error registering <egavgaDriverProc>...");
    exit(0);
  }
#endif
