...
static void safe_line(int x1, int y1, int x2, int y2) {
  ...
  while (!done) {
    ...
    if (code1 == 0 && code2 == 0) {
      ...
      if (code & 1) {
        *yp = map_bounds.y1;
        if (dy) *xp = x1 + dx * (map_bounds.y1 - y1) / dy;
      } else if (code & 2) {
        *yp = map_bounds.y2;
        if (dy) *xp = x1 + dx * (map_bounds.y2 - y1) / dy;
      } else if (code & 4) {
        *xp = map_bounds.x1;
        if (dx) *yp = y1 + dy * (map_bounds.x1 - x1) / dx;
      } else if (code & 8) {
        *xp = map_bounds.x2;
        if (dx) *yp = y1 + dy * (map_bounds.x2 - x1) / dx;
      }
    }
  }
...
static void safe_outtextxy(int x, int y, char* s) {
  if (x >= map_bounds.x1 && x <= map_bounds.x2 &&
      x < map_bounds.x2 - textwidth(s) && y >= map_bounds.y1 &&
      y <= map_bounds.y2 - 60 && y < map_bounds.y2 - textheight(s))
    outtextxy(x, y, s);
}
...
static void drawObjects(int mode) {
  ...
  for (i = 0; i < obj_size; i++) {
    ...
    if (mode == 1) {
      ...
      POINT c = p(obj_list[i].x + offsetX, obj_list[i].y + offsetY,
...
static void draw2dwnd(int isCoord, int isHyper) {
  ...
  for (i = 0; i < sol_size; i++) {
        ...
        if (gs.upgrade_objects_map && obj_size) {
          for (o = 0; o < obj_size; o++) {
            if (core_objects_sphere_line_intersect(
                    sol_list[i].x, sol_list[i].y, sol_list[i].z, buf.x, buf.y,
                    buf.z, obj_list[o].x, obj_list[o].y, obj_list[o].z,
                    obj_list[o].r)) {
              setcolor(4);
              setlinestyle(0, 0, 1);
              safe_line(ex(sol_list[i].x + offsetX),
                        ey(sol_list[i].y + offsetY), ex(buf.x + offsetX),
                        ey(buf.y + offsetY));
              setlinestyle(1, 0, 1);
              break;
            }
    ...
    /** SECTOR NAMES **/
    if (sol_list[i].is_shipyard && sol_list[i].is_gas_station &&
        gs.upgrade_political_map) {
      setcolor(data_factions_colors[sol_list[i].faction]);
      settextstyle(SMALL_FONT, HORIZ_DIR, 4);

      /* Manual politic map aligment*/
      /* "Dhat", "Medinat", "Ghabkar", "Buraq", "Ben Vara", "Killoch Vairan",
       * "Cuchulainn", "Danter", "Coalsack" */
      switch (sol_list[i].sector) {
        case 0:
          /* Dhat */
          safe_outtextxy(ex(sol_list[i].x + offsetX) + 30,
        ...
        case 1:
          /* Medinat */
          safe_outtextxy(ex(sol_list[i].x + offsetX) - 20,
        ...
        case 2:
          /* Ghabkar */
          safe_outtextxy(ex(sol_list[i].x + offsetX) - 60,
        ...
        case 3:
          /* Buraq */
          safe_outtextxy(ex(sol_list[i].x + offsetX) + 27,
        ...
        case 6:
          /* Cuchulainn */
          safe_outtextxy(ex(sol_list[i].x + offsetX) - 115,
        ...
        case 8:
          /* Coalsack */
          safe_outtextxy(ex(sol_list[i].x + offsetX) - 16,
    ...
    for (i = 1; i < wp.size; i++) {
      ...
      for (o = 0; o < obj_size; o++) {
        if (core_objects_sphere_line_intersect(
          ...
          setcolor(4);
          safe_line(
              ex(sol_list[a_idx].x + offsetX), ey(sol_list[a_idx].y + offsetY),
  ...
  for (i = 0; i < sol_size; i++) {
    ...
    if (sol_list[i].threadSize && drawThreads) {
      ...
      for (j = 0; j < sol_list[i].threadSize; j++) {
        buf = sol_list[sol_list[i].threads[j].value];
        {
          POINT A8 = p(buf.x + offsetX, buf.y + offsetY, buf.z + offsetZ);
  ...
  for (i = 0; i < sol_size; i++) {
    ...
    if (sol_list[i].threadSize && drawThreads) {
      for (j = 0; j < sol_list[i].threadSize; j++) {
        ...
        buf = sol_list[sol_list[i].threads[j].value];
        safe_line(ex(sol_list[i].x + offsetX),
                  ey(-1 * (sol_list[i].z + offsetZ)), ex(buf.x + offsetX),
                  ey(-1 * (buf.z + offsetZ)));
