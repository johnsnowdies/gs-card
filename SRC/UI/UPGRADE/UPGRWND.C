...
static void draw_engine_info(void) {
  ...
  sprintf(buf, "%s: %d", LC_UPGRADE_FUEL_PER_JUMP,
          data_hyper_fuel[gs.hyper_class]);
...
void gui_upgrade_draw_list(void) {
  ...
  gui_draw_section_header(upgrade_wnd.x, y_pos, upgrade_wnd.width,
                          LC_UPGRADE_BLACK_MARKET);
  ...
  bar(upgrade_wnd.x + 1, y_pos, upgrade_wnd.x + upgrade_wnd.width - 1,
      y_pos + system_upgrades_size * 10);
  ...
  for (i = 0; i < system_upgrades_size; i++) {
    ...
    if (i == upgrade_selected) {
      ...
      bar(upgrade_wnd.x + 10, y_pos + (10 * i),
          upgrade_wnd.x + upgrade_wnd.width - 11, y_pos + (10 * i) + 10);
      setcolor(BLACK);
    } else {
      setcolor(15);
    }
