...
int data_reader_load_bounds(BOUND_LINE** list) {
  ...
  while (fgets(buf, sizeof(buf), f)) {
    ...
    *q = '\0';
    if (sscanf(p, "%d,%d", &x1, &y1) != 2) continue;
    if (sscanf(q + 1, "%d,%d", &x2, &y2) != 2) continue;
...
int data_reader_load_systems(SYSTEM** list) {
  ...
  *list = (SYSTEM*)calloc(count, sizeof(SYSTEM));
  ...
  while (fgets(buf, sizeof buf, fp) != NULL) {
    ...
    int n = sscanf(buf, "%d;%d;%d;%u;%u;%u;%u", &x, &y, &z, &faction,
                   &is_shipyard, &is_gas_station, &sector);
...
void data_reader_draw_bmp(char* filename, int px, int py) {
  ...
  offbits =
      fgetc(fp) + (fgetc(fp) << 8) + (fgetc(fp) << 16) + (fgetc(fp) << 24);
